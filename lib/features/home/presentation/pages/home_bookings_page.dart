...
// Define the orderServiceProvider for this page
final homeOrderServiceProvider = Provider<OrderService>((ref) {
  final apiClient = ref.watch(apiClientProvider);
  return OrderService(apiClient);
});

// Cache key provider that controls when to refresh the data
final homeOrdersCacheKeyProvider = StateProvider<int>((ref) => 0);

// Store the last successful result
final _homeLastOrdersResultProvider =
...
final homeOrdersProvider =
    FutureProvider.family<Map<String, List<OrderResponse>>, int>(
        (ref, cacheKey) async {
  debugPrint('HomeBookingsPage: Fetching orders with cache key $cacheKey');

  final orderService = ref.watch(homeOrderServiceProvider);
  ...
  ref.read(_homeLastOrdersResultProvider.notifier).state = newResult;
  return newResult;
});

class HomeBookingsPage extends ConsumerStatefulWidget {
  const HomeBookingsPage({
    super.key,
  });

  @override
  ConsumerState<HomeBookingsPage> createState() => _HomeBookingsPageState();
}

class _HomeBookingsPageState extends ConsumerState<HomeBookingsPage>
  ...
  void _refreshData({bool force = false}) {
    if (!mounted || _isRefreshing) {
      debugPrint(
          'HomeBookingsPage: Skipping refresh - not mounted or already refreshing');
      return;
    }

    if (!force && !_shouldRefresh) {
      debugPrint('HomeBookingsPage: Using cached data');
      return;
    }
    ...
    try {
      debugPrint('HomeBookingsPage: Executing refresh');
      _lastRefreshTime = DateTime.now();
      final newCacheKey = _lastRefreshTime!.millisecondsSinceEpoch;

      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ref.read(homeOrdersCacheKeyProvider.notifier).state = newCacheKey;
          debugPrint(
              'HomeBookingsPage: Orders refresh initiated with key $newCacheKey');
        }
      });
    } finally {
      if (mounted) {
  ...
  void _setupRefreshTimer() {
    ...
    if (_tabController.index == 0) {
      _refreshTimer = Timer.periodic(const Duration(minutes: 2), (timer) {
        if (mounted) {
          debugPrint('HomeBookingsPage: Periodic refresh triggered');
          _refreshData(force: true);
        }
      });
    }
  ...
  void didPush() {
    super.didPush();
    debugPrint('HomeBookingsPage: didPush called');
    _refreshData(force: true);
  }
  ...
  void didPopNext() {
    super.didPopNext();
    debugPrint('HomeBookingsPage: didPopNext called');
    _refreshData(force: true);
  }

  void _handleTabChange() {
    if (_tabController.indexIsChanging) {
      debugPrint('HomeBookingsPage: Tab changed to ${_tabController.index}');
      _setupRefreshTimer();
    }
  }
                    ...
                    buildingsAsync.whenData((buildings) {
                      if (buildings['coworking'] != null) {
                        ...
                        if (coworkings.isNotEmpty) {
                          ...
                          context.pushNamed(
                            'coworking_services',
                            pathParameters: {'id': firstCoworkingId.toString()},
                          );
                        } else {
                          context.pushNamed('coworkings');
                        }
                      } else {
                        context.pushNamed('coworkings');
                      }
  ...
  Widget _buildBookingsList(bool isActive) {
    final cacheKey = ref.watch(homeOrdersCacheKeyProvider);
    final ordersAsync = ref.watch(homeOrdersProvider(cacheKey));
